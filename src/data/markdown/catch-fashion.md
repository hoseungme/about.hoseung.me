...
- 주니어 프론트엔드 개발자로써
  ...
  - 프론트엔드를 처음 접하는 것이었음에도 빠르게 적응해 나갔고, 현재는 [CATCH FASHION](https://www.catchfashion.com) 프론트엔드의 모든 영역을 담당하고 있습니다.

  - Client-side cache, Visual Regression Test 도입을 위한 리서치, Lighthouse CI를 활용한 퍼포먼스 측정 및 개선 등등 단순 HTML/JS/CSS 작업 이외에도 웹 개발에 관련된 다양한 경험을 하려 노력했습니다.

- 주니어 백엔드 개발자로써

  - MSA로 설계된 완전히 serverless로 동작하는 백엔드 시스템의 여러 영역을 담당했습니다.

  - 프론트엔드 작업을 동반한 새로운 API 설계, caching 패턴, pagination 패턴, 자동화 패턴, cron 스케줄링, 동시성 제한 등 기존에 미숙했던 여러 부분을 경험했습니다.

- 주니어 개발자로써

  - 작업이 들어오는 대로 그냥 하는게 아니라, 이 작업이 왜 필요한 건지, 무엇을 개선하려는 건지, 유저에게 왜 유의미 한건지 등을 생각하는 연습을 많이 했습니다. 이런 연습이 작업의 범위를 구분할 때, 리소스를 얼마나 들여야하는 작업인지 판단할 때, 새로운 기능을 기획할 때 등등 여러 상황에 많은 도움이 되었습니다.

  - 실제로 직접 작업한 부분은 아니더라도 각각의 기능들과 코드가 어떻게 설계되어 있는지를 자세히 살펴보면서, 깔끔한 기능 설계와 코드의 관심사 분리, 재사용 및 확장이 용이한 코드 등을 고민했습니다.
...
- [활동 리스트](#활동-리스트)
  ...
  - [모달 페이지 스크롤 유지 기능 개발 (2021.05)](#모달-페이지-스크롤-유지-기능-개발-(2021.05))

  - [리테일러 프로모션 포스트들을 '이동 후 구매' 페이지에 노출 (2021.04)](#리테일러-프로모션-포스트들을-'이동-후-구매'-페이지에-노출-(2021.04))

  - ['쇼핑하기' 페이지 프론트엔드 / 백엔드 개발 (2021.04)](#'쇼핑하기'-페이지-프론트엔드-/-백엔드-개발-(2021.04))
...
- **작업 내용 & 배운 점**
  ...
  - 웹훅을 처리하는 Lambda 함수에는 메모리 크기를 적게 할당했기 때문에, [p-limit](https://www.npmjs.com/package/p-limit)등을 통해 Promise.all 등에서 동시성에 제한을 두는 방식을 적용했습니다.
