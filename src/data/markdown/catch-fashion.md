...
- **설명**
  - 기존에는 [react-query](https://www.npmjs.com/package/react-query)를 사용해서 메모리 캐싱을 하고 있었습니다.

  - 하지만 응답 데이터의 변동이 없거나, 변동 빈도가 매우 낮은 API도 첫 접속시에는 요청이 보내진다는 단점이 있었습니다.

  - 따라서 indexedDB, local storage를 사용한 클라이언트 사이드 캐싱을 도입했습니다.
    - persistentCache 옵션이 true로 설정된 API들에 대해, 요청을 보내기 전 캐싱된 데이터가 있는지 확인하도록 하였습니다.
