...
- [활동 리스트](#활동-리스트)
  ...
  - [상품 페이지 리팩토링 (2020.10)](#상품-페이지-리팩토링-(2020.10))

  - [상품 태그 시스템 도입 (2020.09)](#상품-태그-시스템-도입-(2020.09))

  - [새로운 디자인 반영 (2020.08)](#새로운-디자인-반영-(2020.08))
...
- **설명**
  - 기존에 이동 후 구매 페이지와 할인코드 페이지가 따로 분리되어 있었는데, 그것을 하나로 합치고 각종 AB 테스트를 도입했습니다.

- **배운 점 & 어려웠던 점**
  - UI 변경에 따라 할인쿠폰에 추가적인 데이터가 필요해졌습니다.
    - 할인쿠폰 데이터는 구글 스프레드시트에 어드민이 넣어놓은 것들을 백엔드에서 읽어와서 가공한 후 응답하는 방식이었습니다.

    - 스프레드시트에 컬럼을 하나 추가하여 데이터를 삽입하고, 백엔드 로직을 수정했습니다.

    - 항상 데이터베이스를 활용하는 방법만 생각했었는데, 경우에 따라 스프레드시트같은 외부 도구를 유연하게 활용하는 것도 때로는 좋은 방법일 수 있다는 느낌을 받았습니다.
